...
const PostRound = ({route, navigation}) => {
    ...
    const post = async () => {
        ...
        const total = score.userScore.reduce((accum, curr) => accum + Number(curr.score), 0)
        const par = score.holes.reduce((accum, curr) => accum + Number(curr.par), 0)
        data.append("score_string", `${total} (${total - par > 0 ? '+' : ''}${total-par})`)
